...
AGGRESSIVE_NEGOTIATIONS = Talent(
    ...
    is_force_talent=True,
    description="Once per session, perform a Hard (♦️♦️♦️) Lightsaber check to decrease the difficulty of all Negotiation checks for the rest of the encounter by 2.",
)
...
CALL_EM = Talent(
    ...
    description="Do not add ⬛ to combat checks due to the use of the Aim maneuver.",
)
...
CALMING_AURA = Talent(
    ...
    is_active_talent=False,
    is_force_talent=True,
    description="When an opponent targets character with a Force power, reduce 🌗 generated by 1.",
)
...
CORELLIAN_SENDOFF = Talent(
    name='Corellian Sendoff',
    is_active_talent=True,
    is_force_talent=False,
    description="Take a Corellian Sendoff action targeting two ships or vehicles at Close range: make a Hard (♦♦♦) Cool check to case the targets to suffer a minor collision.",
)

CRUCIAL_POINT = Talent(
    name='Crucial Point',
    is_active_talent=True,
    is_force_talent=False,
...
HARD_HEADED = Talent(
    ...
    is_active_talent=True,
    is_force_talent=False,
    description="When staggered or disoriented, perform the Hard Headed action; make a Daunting (♦️♦️♦️♦️) Discipline check to remove status. Difficulty reduced 1 per rank of Hard Headed.",
)

HIDDEN_STORAGE = Talent(
    name='Hidden Storage',
    is_active_talent=False,
    is_force_talent=False,
...
IMPROVED_CORELLIAN_SENDOFF = Talent(
...
IMPROVED_DEAD_TO_RIGHTS = Talent(
...
IMPROVED_HIDDEN_STORAGE = Talent(
...
IMPROVED_INDISTINGUISHABLE = Talent(
...
IMPROVED_SAVVY_NEGOTIATIOR = Talent(
    name='Improved Savvy Negotiatior',
    is_active_talent=True,
    is_force_talent=False,
    description="Make a Hard (♦️♦️♦️) Negotiation check to convince one bystander per rank of Savvy Negotiator to see one of an opponent's points as maliciously unreasonable.",
)

IMPROVED_SUNDER = Talent(
...
INCITE_DISTRACTION = Talent(
    name='Incite Distraction',
    is_active_talent=False,
    is_force_talent=False,
    description="While in a crowd, may perform the Incite Distraction to make an Average (♦️♦️) Deception check if successful, enemies treat the area as difficult terrain. The character may spend Triumph to make the location impassable terrain to them instead.",
)

INDISTINGUISHABLE = Talent(
...
LOSE_THEM = Talent(
    ...
    is_active_talent=True,
    is_force_talent=False,
    description="When being followed or chased, the character may perform the Lose Them action and make a Hard (♦️♦️) Stealth check. If successful, add ⬛ ⬛ to checks to follow him for the remainder of the encounter and may spend Triumph to indicate his pursuers have lost him completely.",
)

MASTER_GRENADIER = Talent(
...
QUICK_STRIKE = Talent(
    ...
    is_active_talent=False,
    is_force_talent=False,
    description="Add 🟦 per rank of Quick Strike to combat checks against targets that have not acted yet this encounter.",
)

RAPID_REACTION = Talent(
    name='Rapid Reaction',
    is_active_talent=True,
    is_force_talent=False,
...
RESEARCHER = Talent(
    ...
    is_active_talent=False,
    is_force_talent=False,
    description="Remove ⬛ per rank of Research from all Knowledge Checks. Researching a subject takes half the time.",
)
...
SAVVY_NEGOTIATOR = Talent(
    name='Savvy Negotiator',
    is_active_talent=False,
    is_force_talent=False,
    description="Remove ⬛ per rank of Savvy Negotiator from all Negotiation and Streetwise checks.",
)

SECOND_CHANCES = Talent(
    name='Second Chances',
    is_active_talent=True,
    is_force_talent=False,
...
SENSE_DANGER = Talent(
    ...
    is_active_talent=True,
    is_force_talent=True,
    description="Once per game, remove ⬛ ⬛ from any 1 check.",
)

SENSE_EMOTIONS = Talent(
    name='Sense Emotions',
    is_active_talent=False,
    is_force_talent=True,
    description="Add 🟦 to all Charm, Coercion, and Deception checks unless the target is immune to Force powers.",
)
...
SHORTCUT = Talent(
    ...
    is_active_talent=False,
    is_force_talent=False,
    description="During a chase, add 🟦 per rank in Shortcut to any checks made to catch or escape an opponent.",
)
...
SKILLED_JOCKEY = Talent(
    name='Skilled Jockey',
    is_active_talent=False,
    is_force_talent=False,
    description="Remove ⬛ per rank of Skilled Jockey from Piloting (Planetary) and Piloting (Space) checks.",
)

SKILLED_TEACHER = Talent(
